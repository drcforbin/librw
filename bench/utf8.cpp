...
void bench_utf8_decoding(ankerl::nanobench::Config& cfg)
{
    ...
    sum = 0;
    cfg.minEpochIterations(40).run("utf8 decoding valid chars (internal)", [&] {
        auto v = "\x24"sv;
        auto [sz, cp] = utf8decode(v);
        sum += cp - sz;
    ...
    sum = 0;
    cfg.minEpochIterations(40).run("utf8 decoding valid chars (mbrtoc32)", [&] {
    ...
    /*
    this doesn't really work the same; it converts all possible
    chars in the input, rather than a single character.
    sum = 0;
